"""用资产锁定关键帧(lk_sX_kf.png) + 加固 Wan5B 重渲全部 6 镜。产物 output/wan5b_assetlock/。"""
...
OUT = Path(r"D:\novel-to-video-codex\output\wan5b_assetlock")
...
async def main():
    ...
    t0 = time.time()
    for shot in SHOTS:
        kf = KF_DIR / f"lk_s{shot['id']}_kf.png"
        if not kf.exists():
        ...
        req = GenerateRequest(
            ...
            output_dir=OUT, output_name=f"lk_shot{shot['id']}", timeout_seconds=2400,
        )
        ts = time.time()
        try:
            ...
            print(f"✗ shot{shot['id']} 失败: {type(exc).__name__}: {str(exc)[:180]}", flush=True)
