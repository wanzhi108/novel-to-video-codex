...
class ComfyUIVideo(BaseTool):
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "workflow_vars": {
                "type": "object",
                "description": (
                    "Optional values for {{VAR}} placeholders in "
    ...
    def execute(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        try:
            if custom_workflow:
                inputs = self._resolve_custom_reference(inputs, output_path)
    ...
    def _resolve_custom_reference(
        self, inputs: dict[str, Any], output_path: Path
    ) -> dict[str, Any]:
        """Upload a local/URL reference image for custom I2V workflows.

        ComfyUI LoadImage nodes address files already on the server, so a
        caller-supplied local path or URL must be uploaded and the resulting
        server-side filename written back into ``INPUT_IMAGE``.
        """
        ref_path = inputs.get("reference_image_path")
        ref_url = inputs.get("reference_image_url")
        ...
        workflow = inputs.get("workflow_json") or ""
        if not workflow and inputs.get("workflow_path"):
            workflow = ComfyUIClient.load_workflow(Path(inputs["workflow_path"]))
        if isinstance(workflow, dict):
            workflow = json.dumps(workflow)
        if "{{INPUT_IMAGE}}" not in workflow:
            raise ComfyUIError(
                "reference_image_path/reference_image_url requires an "
                "{{INPUT_IMAGE}} placeholder in the custom workflow"
            )
        ...
        if not ref_path:
            raise ComfyUIError(
                "custom workflow reference requires reference_image_path "
                "or reference_image_url"
            )

        upload_name = f"om_{output_path.stem}_{uuid.uuid4().hex[:8]}.png"
        server_name = self._client.upload_image(Path(ref_path), upload_name)
        workflow_vars = dict(inputs.get("workflow_vars") or {})
        workflow_vars["INPUT_IMAGE"] = server_name
        inputs["workflow_vars"] = workflow_vars
        return inputs
    ...
    def _load_custom_workflow(inputs: dict[str, Any]) -> dict:
        if inputs.get("workflow_json"):
            ...
            workflow = ComfyUIClient.load_workflow(Path(inputs["workflow_path"]))
        if inputs.get("workflow_vars"):
            workflow = ComfyUIClient.fill_workflow_vars(
                workflow, inputs["workflow_vars"]
            )
        return workflow
