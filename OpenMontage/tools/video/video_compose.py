...
class VideoCompose(BaseTool):
    ...
    def _cuts_to_cinematic_scenes(cuts: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Adapt canonical cuts to CinematicRenderer's scene contract.

        Cuts may carry optional `start_seconds` + `duration_seconds` to
        place a title card over an already-running video beat. When absent,
        scenes stack sequentially from a running timeline cursor.
        """
        ...
        for index, cut in enumerate(cuts):
            try:
                ...
                continue
            source_duration = max(0.0, (source_out - source_in) / speed)

            try:
                explicit_start = float(cut["start_seconds"])
            except (TypeError, KeyError, ValueError):
                explicit_start = None
            try:
                explicit_duration = float(cut["duration_seconds"])
            except (TypeError, KeyError, ValueError):
            ...
            needs_freeze = source_duration > 0 and duration > source_duration + 1e-6
            start = explicit_start if explicit_start is not None else timeline_cursor
            ...
            if cut_type in title_types or not source:
                ...
                vertical_position = str(cut.get("vertical_position") or "center").lower()
                if vertical_position in {"top", "center", "bottom"}:
                    scene["verticalPosition"] = vertical_position
                if source:
                    ...
                    if needs_freeze:
                        scene["backgroundFreezeAfterSeconds"] = source_duration
            else:
                scene = {
                    **common,
                    "kind": "video",
                ...
                if needs_freeze:
                    scene["freezeAfterSeconds"] = source_duration
            ...
            scenes.append(scene)
            timeline_cursor = max(timeline_cursor, start + duration)
    ...
    def _remotion_render(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        if composition_id == "CinematicRenderer":
            ...
            if isinstance(audio_config, dict) and isinstance(audio_config.get("music"), dict):
                music = audio_config["music"]
                if music.get("src"):
                    props["music"] = {
                        "src": music["src"],
                        "volume": music.get("volume", 0.32),
                        "fadeInSeconds": music.get("fade_in_seconds", 2),
                        "fadeOutSeconds": music.get("fade_out_seconds", 3),
                    }
                    trim_after = music.get("trim_after_seconds")
                    if trim_after is not None:
                        props["music"]["trimAfterSeconds"] = trim_after
        ...
        staged_count = self._stage_remotion_media(props, public_dir, project_root=output_path.parent.parent)
