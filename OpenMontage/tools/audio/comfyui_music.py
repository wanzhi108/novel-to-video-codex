...
class ComfyUIMusic(BaseTool):
    ...
    def operation_statuses(self) -> dict[str, str]:
        """Bundled workflow readiness; custom workflows do not depend on it."""
        if not self._client.is_available():
            return {"text_to_audio": "unavailable"}
        _, missing = self._client.check_models(_REQUIRED_MODELS)
        return {"text_to_audio": "available" if not missing else "degraded"}
