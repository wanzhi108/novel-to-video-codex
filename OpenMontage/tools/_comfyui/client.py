...
class ComfyUIClient:
    ...
    def generate(
        ...
        *,
        ...
        # under "gifs", newer video outputs under "videos", and the native
        # SaveAudio node's output under "audio".
        items = (
    ...
    @staticmethod
    def fill_workflow_vars(workflow: dict, workflow_vars: dict[str, Any]) -> dict:
        """Deep-copy *workflow*, filling ``"{{KEY}}"`` template placeholders.

        Values are JSON-encoded so strings stay quoted and numbers stay
        numeric. Raises when a placeholder has no supplied value, so a
        half-filled workflow fails fast instead of hitting ComfyUI as a
        literal model name/prompt.
        """
        s = json.dumps(workflow)
        for key, value in workflow_vars.items():
            s = s.replace(f'"{{{{{key}}}}}"', json.dumps(value))
        missing = sorted(set(re.findall(r'"\{\{([^}]+)\}\}"', s)))
        if missing:
            raise ComfyUIError(
                f"Workflow template variables missing: {', '.join(missing)}"
            )
        return json.loads(s)
