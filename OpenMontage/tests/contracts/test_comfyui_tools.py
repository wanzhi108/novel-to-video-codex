...
class TestClientHelpers:
    ...
    def test_load_workflow_reads_utf8_json(self, tmp_path):
        from tools._comfyui.client import ComfyUIClient
        p = tmp_path / "cn.json"
        p.write_bytes('{"1": {"inputs": {"text": "\u4e00\u53ea\u732b"}}}'.encode("utf-8"))
        assert ComfyUIClient.load_workflow(p)["1"]["inputs"]["text"] == "\u4e00\u53ea\u732b"
    ...
    def test_generate_reads_videos_key_from_savevideo_node(self, monkeypatch, tmp_path):
        from tools._comfyui.client import ComfyUIClient
        ...
        paths = client.generate({"9": {"inputs": {}}}, "9", tmp_path / "out.mp4")
        assert paths == [tmp_path / "out.mp4"]

    def test_fill_workflow_vars_replaces_placeholders_and_preserves_types(self):
        from tools._comfyui.client import ComfyUIClient

        filled = ComfyUIClient.fill_workflow_vars(
            {
                "1": {
                    "class_type": "X",
                    "inputs": {
        ...
        assert filled["1"]["inputs"] == {
            "name": "ltx.safetensors",
            "text": "a cat",
            "seed": 42,
            "count": 3,
            "flag": True,
    ...
    def test_fill_workflow_vars_raises_on_unfilled_placeholder(self):
        from tools._comfyui.client import ComfyUIClient, ComfyUIError

        with pytest.raises(ComfyUIError, match="SEED"):
            ComfyUIClient.fill_workflow_vars(
                {"1": {"inputs": {"seed": "{{SEED}}"}}},
                {"MODEL": "x"},
            )
...
class TestCustomWorkflowContract:
    ...
    def test_video_custom_workflow_fills_template_vars(self):
        tool = ComfyUIVideo()
        workflow = tool._load_custom_workflow(
            {
                "workflow_json": json.dumps({
                    "42": {"inputs": {"text": "{{PROMPT}}", "seed": "{{SEED}}"}}
                }),
                "workflow_vars": {"PROMPT": "hello", "SEED": 7},
            }
        )
        assert workflow["42"]["inputs"] == {"text": "hello", "seed": 7}

    def test_video_custom_workflow_uploads_local_reference(self, tmp_path):
        tool = ComfyUIVideo()
        tool._client.upload_image = lambda local_path, name: f"server-{name}"

        ref = tmp_path / "ref.png"
        ref.write_bytes(b"png")
        inputs = tool._resolve_custom_reference(
            {
                "workflow_json": json.dumps({
                    "42": {"inputs": {"image": "{{INPUT_IMAGE}}"}}
                }),
                "workflow_vars": {"PROMPT": "hello"},
                "reference_image_path": str(ref),
            },
            tmp_path / "shot.mp4",
        )

        server_name = inputs["workflow_vars"]["INPUT_IMAGE"]
        assert server_name.startswith("server-om_shot_")
        assert server_name.endswith(".png")
        workflow = tool._load_custom_workflow(inputs)
        assert workflow["42"]["inputs"]["image"] == server_name

    def test_video_custom_workflow_uploads_url_reference(self, tmp_path, monkeypatch):
        tool = ComfyUIVideo()
        tool._client.upload_image = lambda local_path, name: f"server-{name}"

        class FakeResponse:
            content = b"png-from-url"

            def raise_for_status(self):
                pass

        monkeypatch.setattr(
            "tools.video.comfyui_video.requests.get",
        ...
        inputs = tool._resolve_custom_reference(
            {
                ...
                "reference_image_url": "http://example.test/ref.png",
            },
            tmp_path / "shot.mp4",
        )
        ...
        server_name = inputs["workflow_vars"]["INPUT_IMAGE"]
        assert server_name.startswith("server-om_shot_")
        assert (tmp_path / "shot.ref.png").read_bytes() == b"png-from-url"

    def test_video_custom_workflow_reference_requires_input_image_placeholder(self):
        from tools._comfyui.client import ComfyUIError

        tool = ComfyUIVideo()
        with pytest.raises(ComfyUIError, match="INPUT_IMAGE"):
            tool._resolve_custom_reference(
                {
                    "workflow_json": json.dumps({
                        "42": {"inputs": {"image": "fixed.png"}}
                    }),
                    "reference_image_path": "C:/nope/ref.png",
                },
                Path("C:/nope/shot.mp4"),
            )
...
class TestComfyUIServerStatus:
    ...
    def test_video_available_when_server_up_but_bundled_models_missing(self):
        tool = ComfyUIVideo()
        tool._client.is_available = lambda: True
        tool._client.check_models = lambda required: ([], list(required))
        assert tool.get_status() == ToolStatus.AVAILABLE
        assert tool.operation_statuses() == {
            "text_to_video": "degraded",
            "image_to_video": "degraded",
        }
        assert tool.get_info()["operation_statuses"] == tool.operation_statuses()

    def test_image_available_when_server_up_but_bundled_models_missing(self):
        tool = ComfyUIImage()
        tool._client.is_available = lambda: True
        tool._client.check_models = lambda required: ([], list(required))
        assert tool.get_status() == ToolStatus.AVAILABLE
        assert tool.operation_statuses() == {"text_to_image": "degraded"}
        assert tool.get_info()["operation_statuses"] == {"text_to_image": "degraded"}

    def test_provider_menu_counts_comfyui_available_when_server_reachable(self):
        ...
        assert "comfyui" in caps["video_generation"]["available_providers"]
        assert "comfyui" in caps["image_generation"]["available_providers"]
        assert "comfyui" in caps["music_generation"]["available_providers"]
        assert caps["video_generation"]["configured"] >= 1
...
class TestComfyUIMusic:
    ...
    def test_server_reachable_reports_available_with_degraded_bundled_operation(self):
        tool = ComfyUIMusic()
        tool._client.is_available = lambda: True
        tool._client.check_models = lambda required: ([], list(required))
        assert tool.get_status() == ToolStatus.AVAILABLE
        assert tool.operation_statuses() == {"text_to_audio": "degraded"}
        assert tool.get_info()["operation_statuses"] == {"text_to_audio": "degraded"}
