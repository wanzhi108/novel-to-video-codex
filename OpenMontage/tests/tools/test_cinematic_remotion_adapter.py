...
def test_cinematic_cut_adapter_freezes_short_sources() -> None:
    scenes = VideoCompose._cuts_to_cinematic_scenes([
        {
            "id": "v2",
            "source": "clip.mp4",
    ...
    assert scenes[0]["freezeAfterSeconds"] == pytest.approx(2.041 / 0.5)
    assert scenes[1]["backgroundPlaybackRate"] == 0.5
    assert scenes[1]["backgroundFreezeAfterSeconds"] == pytest.approx(2.041 / 0.5)


def test_remotion_render_maps_cinematic_music(monkeypatch, tmp_path) -> None:
    ...
    result = VideoCompose()._remotion_render({
        "edit_decisions": {
            ...
            "cuts": [
                {"id": "t", "source": "", "type": "hero_title", "text": "Hi", "in_seconds": 0, "out_seconds": 2}
            ],
            "audio": {
                "music": {
    ...
    assert captured["props"]["music"]["fadeInSeconds"] == 2
    assert captured["props"]["music"]["fadeOutSeconds"] == 4
